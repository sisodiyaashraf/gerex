...
void main() {
  ...
  group('Mascot Sprite Sheet Asset Frame Boundary Tests', () {
    for (final state in MascotState.values) {
      test('Verify zero frame border bleed for ${state.name} (${state.assetPath})', () async {
        ...
        for (int i = 0; i < state.frameCount; i++) {
          ...
          expect(leftBorderBleed, 0,
              reason: 'Frame $i in ${state.name} has left border bleed ($leftBorderBleed px)');
          expect(rightBorderBleed, 0,
              reason: 'Frame $i in ${state.name} has right border bleed ($rightBorderBleed px)');
        }
      });
    }
  });
